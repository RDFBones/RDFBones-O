        ...
        for(var i=0; i<3; i++)
        {
            ...
            switch(i){
                ...
                case 1: floatDiv[i].innerHTML=dataSet[n*10+4]+' '+dataSet[n*10];
                        floatDiv[i].setAttribute('style', 'float: left; margin: auto; font-size: 18px; width: 270px;'); 
                        break;
...
function closeFilter(){
    ...
    //filter.setAttribute('style', 'border: 1px solid red; border-radius: 150px;');
    filter.insertBefore(left, filt);
...
function initTree()
{
    ...
    for(var i=-1; i<classes.length; i++){
        ...
        for(var k=start; k<children.length+start; k++){
            ...
            names[k].setAttribute('style','margin: 3px');
...
function showAllCollections(){
    ...
    for(var i=0; i<colldivs.length; i++)
    {
        document.getElementById("collectionContainer").appendChild(colldivs[i]);
    }  
    
    validCollectionDivs.lenght=0;
