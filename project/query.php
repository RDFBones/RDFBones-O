...
$infer = true;                          // Can also choose to explicitly disallow inference. (Optional)
    ...
    switch ($_GET['instruction'])
    {
        ...
        case 'getBones':
            ...
            $query="select ?boneType ?bonearea ?bonefunction
                    where{
                        ?bone rdf:type ?boneType.
                        ?bone predicates:belongsToCollection collection:".$id.".
                        ?boneType rdfs:subClassOf ?bonearea.
                        ?boneType rdfs:subClassOf ?bonefunction.
                        ?bonearea rdfs:subClassOf+ class:BoneAreas.
                        ?bonefunction rdfs:subClassOf+ class:BoneFunctions.
                    }";
            //echo $prefixes.$query;
        ...
        case 'filterCollection':  
            ...
            if($num){
                 $query.="?bone predicates:belongsToCollection ?collection.
                            ?bone rdf:type ?class.";
            };            
            for($i=0; $i<$num; $i++){

               $area=$_POST['area'.$i]; 
               $query.="?class rdfs:subClassOf+ class:".$area.'.'; 
            };
